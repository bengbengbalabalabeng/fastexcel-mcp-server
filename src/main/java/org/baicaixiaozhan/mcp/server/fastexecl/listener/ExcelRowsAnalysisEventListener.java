 ...
 */
@Slf4j
public class ExcelRowsAnalysisEventListener extends ExcelHeadAnalysisEventListener {

    private final Map<Integer, ExcelRowProperties> rowsMap;

    public ExcelRowsAnalysisEventListener() {
        super();
    ...
    public void invoke(Map<Integer, String> data, AnalysisContext context) {
        ...
        Map<Integer, ExcelPropertyHead> headsMap = getHeadsMap();

        ExcelRowProperties row = new ExcelRowProperties();
        row.setRowIndex(rowIndex);
        for (Map.Entry<Integer, String> entry : data.entrySet()) {
            ExcelProperty property = new ExcelProperty();
            ExcelPropertyHead head = headsMap.get(entry.getKey());
            if (Objects.nonNull(head)) {
                property.setHead(head);
            }

            property.setText(entry.getValue());
            row.addProperty(entry.getKey() + 1, property);
        }

        rowsMap.put(rowIndex, row);
    }

    public List<ExcelRowProperties> getRows() {
        return rowsMap.entrySet()
                .stream()
